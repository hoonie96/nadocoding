...
# 캐릭터(스프라이트) 불러오기
character =  pygame.image.load("C:/Users/hooni/Documents/coding/2/pygame_basic/character.png")
...
# 적 enemy 캐릭터
enemy =  pygame.image.load("C:/Users/hooni/Documents/coding/2/pygame_basic/enemy.png")
...
while running:
    dt = clock.tick(60)
    # print("fps : " + str(clock.get_fps())) # 게임화면의 초당 프레임 수를 설정
...
# 20 fps : 1초 동안에 20번 동작 -> 1번에 5만큼 5 * 20 = 100
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.KEYDOWN: # 키가 눌러졌는지 확인
            if event.key == pygame.K_LEFT: # 캐틱터를 왼쪽으로
                to_x -= character_speed # to_x = to_x -5
            elif event.key == pygame.K_RIGHT: # 캐틱터를 오른쪽으로
                to_x += character_speed
            elif event.key == pygame.K_UP: # 캐틱터를 위로
                to_y -= character_speed
            elif event.key == pygame.K_DOWN: # 캐틱터를 아래로
                to_y += character_speed
    ...
    screen.blit(background, (0, 0)) # 배경 그리기. x, y 축
